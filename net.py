...
class InferenceNetwork(nn.Module):
    """
    The inference network for the hardware-adaptive modulator z. For more details cf. https://arxiv.org/abs/1905.12917
    TODO: low-priority: understand how this works
    """
    ...
    def get_posterior(self, inputs):
        ...
        ].squeeze()  # even indices for mean
        sigma_z = z_stats[
            (self.layer_size * 2 + 1) * 3 :
        ].squeeze()  # odd indices for sigma
...
def kl_diagnormal_stdnormal(p):
    ...
    That is, it computes the kl divergence of p from q where q is a (multivariate) standard normal distribution.
    """
    pshape = p.mean.shape
    device = p.mean.device
    q = torch.distributions.Normal(
        torch.zeros(pshape, device=device), torch.ones(pshape, device=device)
    )
    return torch.distributions.kl.kl_divergence(p, q).to(device)


class MetaLearner(nn.Module):
    def __init__(self, search_space, hw_embed_on, hw_embed_dim, layer_size):
        super(MetaLearner, self).__init__()

        if search_space == "nasbench201":
            self.meta_learner = GCN(
                nfeat=8,
...
class Net(nn.Module):
    ...
    def forward(self, x, hw_embed=None, params=None):
        ...
        if params == None:
            ...
            if self.hw_embed_on:
                # Passes the input hardware embedding through a linear layer and concats it to the output of the
                # of the first two linear layers for the architecture embedding.
                hw = self.relu(self.fc_hw1(hw_embed))
                hw = self.relu(self.fc_hw2(hw))
                out = torch.cat([out, hw], dim=-1)

            # Passes the combined architecture and hardware embedding through the last three linear layers.
            ...
            out = F.relu(
                F.linear(
                    x,
                    params["meta_learner.fc1.weight"],
            ...
            out = F.relu(
                F.linear(
                    out,
                    params["meta_learner.fc2.weight"],
            ...
            if self.hw_embed_on:
                hw = F.relu(
                    F.linear(
                        hw_embed,
                        params["meta_learner.fc_hw1.weight"],
                ...
                hw = F.relu(
                    F.linear(
                        hw,
                        params["meta_learner.fc_hw2.weight"],
            ...
            out = F.relu(
                F.linear(
                    out,
                    params["meta_learner.fc3.weight"],
            ...
            out = F.relu(
                F.linear(
                    out,
                    params["meta_learner.fc4.weight"],
                    params["meta_learner.fc4.bias"],
                )
            )
            out = F.linear(
                out, params["meta_learner.fc5.weight"], params["meta_learner.fc5.bias"]
            )
...
class GCN(nn.Module):
    """
    The base model for MAML (Meta-SGD) for meta-NAS-predictor.
    Only used in combination with the NASBench201 search space.
    """
        ...
        if params == None:
            ...
            out = F.relu(
                self.gc1(
                    feat,
                    adj,
                    weight=params["meta_learner.gc1.weight"],
                    bias=params["meta_learner.gc1.bias"],
                ).transpose(2, 1)
            )
            out = out.transpose(1, 2)
            out = F.relu(
                self.gc2(
                    out,
                    adj,
                    weight=params["meta_learner.gc2.weight"],
                    bias=params["meta_learner.gc2.bias"],
                ).transpose(2, 1)
            )
            out = out.transpose(1, 2)
            out = F.relu(
                self.gc3(
                    out,
                    adj,
                    weight=params["meta_learner.gc3.weight"],
                    bias=params["meta_learner.gc3.bias"],
                ).transpose(2, 1)
            )
            out = out.transpose(1, 2)
            out = F.relu(
                self.gc4(
                    out,
                    adj,
                    weight=params["meta_learner.gc4.weight"],
                    bias=params["meta_learner.gc4.bias"],
                ).transpose(2, 1)
            )
            out = out.transpose(1, 2)
            out = out[:, out.size()[1] - 1, :]
