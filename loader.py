...
class Data:
    def __init__(
        ...
        for device in meta_train_devices + meta_valid_devices + meta_test_devices:
            ...
            self.norm_latency[device] = normalization(
                latency=self.latency[device], index=self.train_idx[device]
            )

        # load index set of reference architectures, i.e., indices of reference architectures
        ...
        if self.mode == "nas":
            ...
            for device in meta_train_devices + meta_valid_devices + meta_test_devices:
                self.nas_norm_latency[device] = normalization(
                    ...
                    index=torch.tensor(
                        [self.max_lat_idx, self.min_lat_idx] + self.hw_emb_idx
                    ),
                )
    ...
    def load_archs(self):
        if self.search_space == "nasbench201":
            ...
            self.archs = [
                [
                    add_global_node(_["operation"], ifAdj=False),
            ...
            self.archs = [
                arch_enc(_["op_idx_list"])
                for _ in torch.load(os.path.join(self.data_path, "metainfo.pt"))["arch"]
            ]
        elif self.search_space == "ofa":
            self.archs = [
                arch_encoding_ofa(arch)
                for arch in torch.load(os.path.join(self.data_path, "ofa_archs.pt"))[
                    "arch"
                ]
            ]
    ...
    def generate_episode(self):
        ...
        # meta-batch
        rand_device_idx = torch.randperm(len(self.meta_train_devices))[
            : self.num_inner_tasks
        ]
        for t in rand_device_idx:
            # sample devices
            device = self.meta_train_devices[t]

            # latencies of all architectures on the device
            latency = self.latency[device]
            # "create" hardware embedding by retrieving the latency of the reference architectures on the device
            hw_embed = latency[self.hw_emb_idx]
            # normalize the hardware embedding
            hw_embed = normalization(hw_embed, portion=1.0)

            # samples for finetuning & test (query)
            rand_idx = self.train_idx[device][
                torch.randperm(len(self.train_idx[device]))
            ]
            finetune_idx = rand_idx[: self.num_sample]
            qry_idx = rand_idx[
                self.num_sample: self.num_sample + self.num_query_meta_train_task
            ]
            ...
            if self.search_space in ["fbnet", "ofa"]:
                ...
                # two matrices for each architecture
                x_finetune = [
                    torch.stack([self.archs[_][0] for _ in finetune_idx]),
                    torch.stack([self.archs[_][1] for _ in finetune_idx]),
                ]
                x_qry = [
                    torch.stack([self.archs[_][0] for _ in qry_idx]),
    ...
    def get_task(self, device=None, num_sample=None):
        ...
        if self.search_space in ["fbnet", "ofa"]:
            ...
            x_qry = [
                torch.stack([self.archs[_][0] for _ in self.valid_idx[device]]),
    ...
    def _load_arch_candidates(self):
        ...
        self.arch_candidates = {"arch": [], "true_acc": []}
    ...
    def get_nas_task(self, device=None):
        ...
        if self.search_space == "nasbench201":
            ...
            x_qry = [
                torch.stack([self.archs[_][0] for _ in metad2a_idx]),
